...
public final class LyricsService extends Service implements AppState.Listener {
    ...
    private AppState state;private SettingsStore settings;private LyricsDb db;private LyricsRepository repo;private TelemetryProcessor processor;private LocalServer server;private MediaSessionMonitor media;private long lastNotificationAt=0;private String lastNotificationTrack="";
    ...
    private void ensureServer(){if(server!=null)return;server=new LocalServer(this);try{server.start();}catch(Exception e){state.log.add("Server failed: "+e.getMessage());}}
    private void shutdown(){if(media!=null)media.stop();if(server!=null){server.stop();server=null;}state.setServiceRunning(false);state.removeListener(this);}
    ...
    private Notification notification(){TrackMetadata t=state.trackCopy();String text=(t.title.isEmpty()?"等待网易云音乐":t.title+" - "+t.artist)+"  "+NetworkUtils.bestLanAddress()+":8765";Intent open=new Intent(this,MainActivity.class);PendingIntent pi=PendingIntent.getActivity(this,0,open,PendingIntent.FLAG_IMMUTABLE|PendingIntent.FLAG_UPDATE_CURRENT);return new Notification.Builder(this,"tesla_lyrics").setContentTitle("Tesla Lyrics 正在运行").setContentText(text).setSmallIcon(android.R.drawable.ic_media_play).setContentIntent(pi).setOngoing(true).build();}
