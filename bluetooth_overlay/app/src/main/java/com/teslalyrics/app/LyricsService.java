...
public final class LyricsService extends Service implements AppState.Listener {
    ...
    private AppState state;private SettingsStore settings;private LyricsDb db;private LyricsRepository repo;private TelemetryProcessor processor;private LocalServer server;private MediaSessionMonitor media;private RemoteControlBridge remote;private MdnsResponder mdns;private long lastNotificationAt=0;private String lastNotificationTrack="";
    @Override public void onCreate(){super.onCreate();state=AppState.get();settings=new SettingsStore(this);db=new LyricsDb(this);repo=new LyricsRepository(db);processor=new TelemetryProcessor(repo);media=new MediaSessionMonitor(this,processor);remote=new RemoteControlBridge(media);state.setGlobalOffsetMs(settings.globalOffset());state.addListener(this);createChannel();}
    @Override public int onStartCommand(Intent i,int flags,int id){String a=i==null?ACTION_START:i.getAction();if(a==null)a=ACTION_START;if(ACTION_STOP.equals(a)){shutdown();stopSelf();return START_NOT_STICKY;}promote();ensureServer();state.setServiceRunning(true);media.start();if(remote!=null)remote.start();if(ACTION_RESYNC.equals(a)){TrackMetadata t=state.trackCopy();if(!t.title.isEmpty())repo.load(t);media.scan();}else if(ACTION_SCAN.equals(a)){media.scan();}else if(ACTION_GLOBAL.equals(a)){long v=i.getLongExtra("ms",0);state.setGlobalOffsetMs(v);settings.setGlobalOffset(v);}else if(ACTION_TRACK.equals(a)){long v=i.getLongExtra("ms",0);state.setTrackOffsetMs(v);db.updateTrackOffset(state.trackCopy().cacheKey(),v);}return START_STICKY;}
    ...
    private void shutdown(){if(remote!=null)remote.stop();if(media!=null)media.stop();if(mdns!=null){mdns.stop();mdns=null;}if(server!=null){server.stop();server=null;}state.setServiceRunning(false);state.removeListener(this);}
    ...
    private Notification notification(){TrackMetadata t=state.trackCopy();String text=(t.title.isEmpty()?"等待网易云音乐":t.title+" - "+t.artist)+"  hanz316.github.io/lyrics/";Intent open=new Intent(this,MainActivity.class);PendingIntent pi=PendingIntent.getActivity(this,0,open,PendingIntent.FLAG_IMMUTABLE|PendingIntent.FLAG_UPDATE_CURRENT);return new Notification.Builder(this,"tesla_lyrics").setContentTitle("Tesla Lyrics 正在运行").setContentText(text).setSmallIcon(android.R.drawable.ic_media_play).setContentIntent(pi).setOngoing(true).build();}
